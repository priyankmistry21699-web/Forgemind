...
def upgrade() -> None:
    ...
    arch_node_type = sa.Enum(
        "workspace", "project", "repository", "package", "module",
        "service", "component", "api", "interface", "datastore",
        "resource", "external_dependency",
        name="arch_node_type",
    )
    arch_node_type.create(op.get_bind(), checkfirst=True)

    arch_edge_type = sa.Enum(
        "depends_on", "calls", "owns", "reads", "writes",
        "exposes", "imports", "deploys_to", "emits_event_to",
        "consumes_event_from",
        name="arch_edge_type",
    )
    ...
    arch_rule_category = sa.Enum(
        "import", "layer", "ownership", "dependency", "boundary",
    ...
    op.create_table(
        ...
        sa.Column("source_type", arch_source_type, nullable=False,
                  server_default="inferred"),
        sa.Column("status", arch_node_status, nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "from_node_id", UUID(as_uuid=True),
            sa.ForeignKey("architecture_nodes.id", ondelete="CASCADE"),
            nullable=False, index=True,
        ),
        sa.Column(
            "to_node_id", UUID(as_uuid=True),
            sa.ForeignKey("architecture_nodes.id", ondelete="CASCADE"),
            nullable=False, index=True,
        ),
        ...
        sa.Column("confidence_score", sa.Float(), nullable=False,
                  server_default="1.0"),
        sa.Column("metadata", JSON, nullable=True),
        sa.Column("source_type", arch_edge_source_type, nullable=False,
                  server_default="inferred"),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.text("now()")),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "source_snapshot_id", UUID(as_uuid=True),
    ...
    op.create_table(
        ...
        sa.Column("severity", arch_rule_severity, nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "target_node_id", UUID(as_uuid=True),
