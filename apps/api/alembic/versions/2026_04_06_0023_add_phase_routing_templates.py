...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("default_governance_config", postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column("default_phase_profiles", postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column("suggested_task_types", postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column("spec_defaults", postgresql.JSON(astext_type=sa.Text()), nullable=True),
        sa.Column("plan_defaults", postgresql.JSON(astext_type=sa.Text()), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column(
            "phase",
            sa.Enum("specify", "plan", "tasks", "implement", "review", "validate", name="workflowphase"),
            nullable=False,
        ),
    ...
    op.create_index("ix_phase_agent_profiles_project_id", "phase_agent_profiles", ["project_id"])
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("pending", "accepted", "rejected", "expired", name="suggestionstatus"),
            nullable=False,
    ...
    op.create_index("ix_constitution_suggestions_project_id", "constitution_suggestions", ["project_id"])


def downgrade() -> None:
    op.drop_index("ix_constitution_suggestions_project_id", table_name="constitution_suggestions")
    op.drop_table("constitution_suggestions")
    op.drop_index("ix_phase_agent_profiles_project_id", table_name="phase_agent_profiles")
