...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "entity_type",
            sa.Enum(
                "task", "artifact", "comment", "run", "project",
                "knowledge", "annotation", "approval", "release_package", "spec",
    ...
    op.create_index("ix_search_index_entity", "search_index", ["entity_type", "entity_id"], unique=True)
    ...
    op.create_table(
        ...
        sa.Column(
            "category",
            sa.Enum("naming", "architecture", "quality", "security", "documentation", name="conventioncategory"),
            nullable=False,
        ),
        ...
        sa.Column(
            "enforcement_level",
            sa.Enum("advisory", "recommended", "required", name="conventionenforcement"),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "rec_type",
            sa.Enum(
                "knowledge_gap", "stale_run", "similar_project",
                "convention_violation", "missing_approval", "reusable_pattern", "tech_debt",
    ...
    op.add_column(
        "artifacts",
        sa.Column("parent_version_id", UUID(as_uuid=True), sa.ForeignKey("artifacts.id", ondelete="SET NULL"), nullable=True),
    )
    op.add_column(
        "artifacts",
